...
class OGMNeo {
    ...
    /**
        ...
        * @param {string} [protocol=bolt] - Neo4j protocol.
        * @param {string} [configuration=undefined] - Optional extra configuration for neo4j driver.

    */
    static connect(user = 'neo4j', password = 'neo4j', host = 'localhost', protocol = 'bolt', configuration = undefined) {
        if (driver == null) {
            driver = neo4j.driver(`${protocol}://${host}`, neo4j.auth.basic(user, password), configuration);
